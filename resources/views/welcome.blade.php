...
@section('content')
    ...
    <section class="py-5">
        <header>
            <p class="small text-muted small text-uppercase mb-1">Destaques</p>
            <h2 class="h5 text-uppercase mb-4">Lojas</h2>
        </header>
        <div class="row">
            <!-- Lojas-->
            ...
            <div class="col-xl-3 col-lg-4 col-sm-6">
                <div class="product text-center">
                    <div class="position-relative mb-3">
                        <div class="badge text-white badge-"></div>
                            <a class="d-block" href="{{route('store.single', ['slug' => $store->slug])}}">
                                @if($store->logo)
                                    <img src="{{asset('storage/' . $store->logo)}}" alt="Logo da Loja {{$store->name}}" class="img-fluid w-100">
                                @else
                                    <img src="{{asset('assets/img/no-photo.jpg')}}" alt="Logo da Loja {{$store->name}}" class="img-fluid w-100">
                                @endif
                            </a>
                            <div class="product-overlay">
                                <ul class="mb-0 list-inline">                                    
                                    <li class="list-inline-item m-0 p-0">
                                        <a class="btn btn-sm btn-dark" href="{{route('store.single', ['slug' => $store->slug])}}">Ver Loja</a>
                                    </li>                                    
                                </ul>
                            </div>
                    </div>
                    <h6> <a class="reset-anchor" href="{{route('store.single', ['slug' => $store->slug])}}">{{$store->name}}</a></h6>
                    <p>
                        {{$store->description}}
                    </p>
                </div>
